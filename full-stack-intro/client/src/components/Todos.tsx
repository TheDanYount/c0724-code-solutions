...
export function Todos() {
  ...
  useEffect(() => {
    const allTodos = async () => {
      try {
        ...
        if (!result.ok) throw new Error('fetch for todos failed');
        const formattedResult = await result.json();
        setTodos(formattedResult);
      } catch (err) {
        setError(err);
      } finally {
        setIsLoading(false);
  ...
  async function addTodo(newTodo: UnsavedTodo) {
    try {
      const todoAddResult = await fetch('/api/todos', {
        method: 'POST',
        headers: {
  ...
  async function toggleCompleted(todo: Todo) {
    try {
      ...
      const todoUpdateResult = await fetch(`/api/todos/${todo.todoId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
